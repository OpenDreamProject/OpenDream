    ...
    class DreamMapManager : IDreamMapManager {
        ...
        public void Initialize() {
            ...
            if (worldDefinition.Variables["area"].TryGetValueAsPath(out var area))
            {
                ...
                if(!_dreamManager.ObjectTree.GetObjectDefinition(area).IsSubtypeOf(_dreamManager.ObjectTree.GetObjectDefinition(DreamPath.Area))) throw new Exception("bad area");
                _defaultArea = area;
            ...
            {
                if(!_dreamManager.ObjectTree.GetObjectDefinition(turf).IsSubtypeOf(_dreamManager.ObjectTree.GetObjectDefinition(DreamPath.Turf))) throw new Exception("bad turf");
                _defaultTurf = turf;
            }
