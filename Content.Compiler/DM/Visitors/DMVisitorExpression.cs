...
namespace Content.Compiler.DM.Visitors {
    public class DMVisitorExpression : DMASTVisitor {
        DMObject _dmObject { get; }
        DMProc _proc { get; }
        DreamPath? _inferredPath { get; }
        internal DMExpression Result { get; private set; }

        public DMVisitorExpression(DMObject dmObject, DMProc proc, DreamPath? inferredPath) {
            _dmObject = dmObject;
            _proc = proc;
        ...
        public void HandleCompileErrorException(CompileErrorException exception) {
            ...
            System.Console.WriteLine(exception.Error.Message);
        }
    }
}
