...
namespace OpenDreamRuntime.Procs.Native {
    ...
    internal static class DreamProcNativeHelpers {
        ...
        static public IEnumerable<DreamObject> MakeViewSpiral(DreamObject center, ViewRange distance) {
            ...
            int donutCount = Math.Max(distance.Width, distance.Height); // TODO: Make rectangles work.
        ...
        static public (DreamObject, ViewRange) ResolveViewArguments(DreamObject usr, DreamProcArguments arguments) {
            ...
            DreamValue maybeDistance;
            bool distanceArgumentNotFound = true;
            if(arguments.TryGetPositionalArgument(0,out maybeDistance)) {
                switch (maybeDistance.Type) {
                    case DreamValue.DreamValueType.Float:
                        int rangeValue = maybeDistance.MustGetValueAsInteger();
                        range = new ViewRange(rangeValue, rangeValue);
                        distanceArgumentNotFound = false;
                        break;
                    case DreamValue.DreamValueType.String:
                        range = new ViewRange(maybeDistance.MustGetValueAsString());
                        distanceArgumentNotFound = false;
                        break;
                }
                
            }
            if (distanceArgumentNotFound && arguments.TryGetNamedArgument("Dist", out maybeDistance)) { // An OD extension - BYOND doesn't support named args for these procs
                switch (maybeDistance.Type) {
                    case DreamValue.DreamValueType.Float:
                        int rangeValue = maybeDistance.MustGetValueAsInteger();
                        range = new ViewRange(rangeValue, rangeValue);
            ...
            //Try to get the centre
            DreamValue maybeCenter;
            bool centerArgumentNotFound = true;
            if (distanceArgumentNotFound && arguments.TryGetPositionalArgument(0, out maybeCenter)) {
                if(maybeCenter.TryGetValueAsDreamObject(out center)) {
                    centerArgumentNotFound = false;
                }
            }
            if(centerArgumentNotFound && arguments.TryGetPositionalArgument(1, out maybeCenter)) {
                if (maybeCenter.TryGetValueAsDreamObject(out center)) {
                    centerArgumentNotFound = false;
                }
            }
            if(centerArgumentNotFound && arguments.TryGetNamedArgument("Center", out maybeCenter)) {
                maybeCenter.TryGetValueAsDreamObject(out center);
            }

            if (distanceArgumentNotFound)
                return (center, new ViewRange(5, 5));
            else
                return (center, range);
        }
