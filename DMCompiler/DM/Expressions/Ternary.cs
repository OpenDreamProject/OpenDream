...
internal sealed class Ternary : DMExpression {
    ...
    public Ternary(Location location, DMExpression a, DMExpression b, DMExpression c) : base(a.Compiler, location) {
        _a = a;
        _b = b;
        _c = c;

        if (b.ValType.TypePath != null && c.ValType.TypePath != null && b.ValType.TypePath != c.ValType.TypePath) {
            a.Compiler.Emit(WarningCode.LostTypeInfo, Location,
...
// var in x to y
internal sealed class InRange(Location location, DMExpression var, DMExpression start, DMExpression end) : DMExpression(var.Compiler, location) {
    public override void EmitPushValue(DMObject dmObject, DMProc proc) {
